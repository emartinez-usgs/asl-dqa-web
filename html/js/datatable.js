    ...
    dTable.find('tbody td').click(function() {
        ...
        if (id != undefined) {
            if (id != undefined && id.charAt(0) == 'd')
                ...
                window.location = 'dataq.html?station=' + id.split('_')[1] + '&sdate=' + getStartDate('simple') + '&edate=' + getEndDate('simple') + '&tdate=' + getDateType();
            }
        }
    });
    ...
    if (pageType == "summary") {
        ...
        for (station in mapSIDtoNID) {
            if (mapSIDtoNID.hasOwnProperty(station)) {
                var row = $(
                    '<tr id = "' + station + '"><td>' + mapGIDtoGName[mapSIDtoNID[station]] + '</td>' + '<td id="l_' + station + '" class="ltd">' + mapSIDtoSName[station] + '</td>' + '<td>,' + mapSIDtoGIDs[station] + ',</td></tr>'
                );
        ...
        for (channel in mapCIDtoCName) {
            if (mapCIDtoCName.hasOwnProperty(channel)) {
                var row = $(
                    '<tr id = "' + channel + '"><td>' + mapCIDtoLoc[channel] + '</td>' + '<td>' + mapCIDtoCName[channel] + '</a></td></tr>'
                );
                //Adding 1.01 causes datatables to automatically set the column types to numeric
                for (var i = 0; i < metricsSorted.length; i++) {
...
//over-rides filtering method and removes empty rows
